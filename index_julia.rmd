...
# code.dir=here("/Users/juliachen/Desktop/covid19_cities/code/")
# data.dir=here("/Users/juliachen/Desktop/covid19_cities/data/")
# result.dir = here("/Users/juliachen/Desktop/covid19_cities/results/")
# fig.dir = here("/Users/juliachen/Desktop/covid19_cities/figs/")
# output.dir = here("/Users/juliachen/Desktop/covid19_cities/output/")
# code.paper.dir=here("/Users/juliachen/Desktop/covid19_cities/code/epi_model_code")
# code.risk.dir=here("/Users/juliachen/Desktop/covid19_cities/code/risk_model_code/")
# code.scenarios.dir=here("/Users/juliachen/Desktop/covid19_cities/code/scenarios_code/")
...
##CITY DATA
city_data <- read_city_data("Los Angeles - Boyle Heights")
city_pop <<- population_data("Los Angeles - Boyle Heights") # global variable 
```
...
for (i in week.no){
  prior.R0 = c("normal", prior_readin[i,3], 0.0009*prior_readin[i,3])
  ...
  source(model_functions)
    ...
    summarydata <- sum.stats.SIMTEST(city_data[c(1:(7*i+1)),])
    ...
    for (j in 1:nrow(ABC.par.out_week)){
        ABC.par.out_week$V1[j] <- max(ABC.par.out_week$V1[j] , .0015)
        ABC.par.out_week$V2[j] <- max(ABC.par.out_week$V2[j] , .007)
        ABC.par.out_week$V3[j] <- max(ABC.par.out_week$V3[j] , .007)
        ABC.par.out_week$V4[j] <- max(ABC.par.out_week$V4[j] , .007)
        ABC.par.out_week$V5[j] <- max(ABC.par.out_week$V5[j] , .007)
        ABC.par.out_week$V6[j] <- max(ABC.par.out_week$V6[j] , .007)
    }
...
#traj.0$date <- traj.0$date + 45
#traj.0$date <- traj.0$date - 45

###################################################################################################
## Plot all variables current date
#time.steps.4.plot = as.Date("2020-04-07") - as.Date("2020-03-01") 
time.steps.4.plot = endDatePlotGlobal - as.Date("2020-03-01")
data.in <- city_data # %>% dplyr::filter(date < "2020-04-20")  #la_data #latest_data
vars.to.plot <- vars.plus.R
...
```{r}
#data.in.short = city_data %>% dplyr::filter(date < as.Date("2020-04-20"))

#plot.model.single(traj.CI=traj.0, data.in=data.in.short, init.date.data=NULL, time.steps.4plot=time.steps.4.plot, ymax=NULL, plot.capacity=NULL, plot.annotations=NULL, var.to.plot="Idetectcum")
```
